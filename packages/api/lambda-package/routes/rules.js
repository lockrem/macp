...
exports.registerRulesRoutes = registerRulesRoutes;
exports.getRulesForPrompt = getRulesForPrompt;
exports.getBulkRulesForPrompt = getBulkRulesForPrompt;
exports.saveExtractedRules = saveExtractedRules;
...
// Helper Functions
// -----------------------------------------------------------------------------
async function getRulesIndex(userId) {
...
async function saveRulesIndex(index) {
...
async function getAgentRules(userId, agentId) {
...
async function saveAgentRules(rules) {
...
async function deleteAgentRulesFile(userId, agentId) {
    const key = `rules/${userId}/${agentId}.json`;
...
// Validation Schemas
// -----------------------------------------------------------------------------
const addRuleSchema = zod_1.z.object({
...
const bulkRulesSchema = zod_1.z.object({
    agentIds: zod_1.z.array(zod_1.z.string()),
});
// -----------------------------------------------------------------------------
// Routes
// -----------------------------------------------------------------------------
function registerRulesRoutes(app) {
    // -------------------------------------------------------------------------
 ...
 */
async function getRulesForPrompt(userId, agentId) {
 ...
 */
async function getBulkRulesForPrompt(userId, agentIds) {
    const results = {};
    await Promise.all(agentIds.map(async (agentId) => {
 ...
 */
async function saveExtractedRules(userId, agentId, agentName, extractedRules) {
    ...
    const existingContents = new Set(agentRules.rules.map(r => r.content.toLowerCase().trim()));
    let added = 0;
    let duplicates = 0;
