...
async function saveOrchestratedConversationMemories(state, apiClient) {
    ...
    try {
        ...
        if (apiClient && (agentDistributions.length > 0 || visitorMemoriesSaved > 0)) {
            try {
...
async function detectTaskCompletion(apiClient, connectionId, state, hostAgent, hostResponse) {
    ...
    const prompt = `Analyze if the host's response indicates any of these pending tasks have been COMPLETED or CONFIRMED:
...
If the host CONFIRMED or COMPLETED a task (e.g., "reservation confirmed", "booked for", "all set for", etc.), respond with:
{
  "completed": true,
  "taskId": "the-task-id",
  "summary": "Brief summary of what was confirmed (e.g., 'Reservation confirmed for 4 at 5:30 PM tomorrow')"
}

If NO task was completed, respond with:
{
  "completed": false
}
    ...
    try {
        ...
        if (result.completed && result.taskId) {
            ...
            if (task) {
                ...
                // Generate brief summaries from each participating agent
                const agentSummaries = await generateAgentSummaries(state, hostAgent, task.description, result.summary);
                // Notify client that task was completed with agent summaries
                await sendToConnection(apiClient, connectionId, {
                    type: 'taskCompleted',
                    ...
                    agentSummaries, // Each agent's contribution summary
                });
                // Send each agent's summary as a turn so it appears in the chat
                for (const agentSummary of agentSummaries) {
                    await sendToConnection(apiClient, connectionId, {
                        ...
                        turn: {
                            ...
                            emoji: agentSummary.emoji,
                            content: agentSummary.summary,
                            timestamp: new Date().toISOString(),
                        },
                    });
                }
