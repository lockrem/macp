...
const createSessionSchema = z.object({
  mode: z.enum(['direct', 'agent_to_agent', 'accompanied']),
  visitorId: z.string(),
  visitorUserId: z.string().optional(),
  visitorAgentId: z.string().optional(),
  visitorAgentName: z.string().optional(),
});

const sendMessageSchema = z.object({
  sessionId: z.string(),
  content: z.string().min(1).max(10000),
  role: z.enum(['user', 'visitor_agent']).default('user'),
...
export function registerPublicAgentRoutes(app: FastifyInstance): void {
  ...
  app.get('/public/agent/:agentId', async (req, reply) => {
    ...
    try {
      ...
      incrementViewCount(agentId).catch(err => {
  ...
  app.post('/public/agent/:agentId/session', async (req, reply) => {
    ...
    try {
      ...
      const session = await createPublicSession(agentId, body);
  ...
  app.post('/public/agent/:agentId/message', async (req, reply) => {
    ...
    try {
      ...
      const { userMessage, agentMessage } = await sendPublicMessage(
        body.sessionId,
        body.content,
        body.role,
        body.apiKey,
  ...
  app.post('/public/agent/:agentId/complete', async (req, reply) => {
    ...
    try {
      ...
      const result = await completePublicSession(
        body.sessionId,
        body.apiKey,
        body.provider
      );
  ...
  app.post('/api/agents/:agentId/publish', async (req, reply) => {
    ...
    try {
      ...
      const agentConfig: AgentConfigForPublishing = {
      ...
      const validation = validatePublishRequest(agentConfig, {
      ...
      const publishedAgent = await publishAgent(
        userId,
        req.user.name,
        agentConfig,
        {
      ...
      return {
        ...
        url: getPublicAgentUrl(publishedAgent.agentId),
        agent: {
          agentId: publishedAgent.agentId,
  ...
  app.put('/api/agents/:agentId/publish', async (req, reply) => {
    ...
    try {
      ...
      const updatedAgent = await updatePublishedAgent(userId, agentId, body);
  ...
  app.delete('/api/agents/:agentId/publish', async (req, reply) => {
    ...
    try {
      await unpublishAgent(userId, agentId);
  ...
  app.get('/api/agents/published', async (req, reply) => {
    ...
    try {
      ...
      const agentsWithDetails = await Promise.all(
        index.agents.map(async (meta) => {
          const agent = await getPublishedAgent(meta.agentId);
          return {
            ...
            url: getPublicAgentUrl(meta.agentId),
  ...
  app.get('/api/agents/published/:agentId/sessions', async (req, reply) => {
    ...
    try {
      const sessions = await getSessionsForAgent(
        userId,
        agentId,
  ...
  app.get('/api/agents/:agentId/publish', async (req, reply) => {
    ...
    try {
      ...
      return {
        isPublished: true,
        agentId: agent.agentId,
        url: getPublicAgentUrl(agent.agentId),
