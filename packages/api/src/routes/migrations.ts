...
export function registerMigrationRoutes(app: FastifyInstance): void {
  ...
  app.post('/api/admin/migrate', async (req, reply) => {
    ...
    try {
      ...
      async function runMigrationSql(name: string, migrationSql: string) {
      ...
      async function runStatements(name: string, statements: string[]) {
      ...
      // Backfill users table from existing data
      console.log('[Migration] Backfilling users table from existing data...');
      ...
      `);
      console.log('[Migration] Users backfill complete');
      results.push('users_backfill: completed');
  ...
  app.get('/api/admin/migrate/status', async (req, reply) => {
    try {
      ...
      const result = await db.execute(sql`
        ...
        AND table_name IN ('users', 'public_agents', 'contacts', 'contact_agents', 'tasks', 'forms', 'form_fields', 'form_submissions', 'form_responses', 'user_memory_facts')
        ORDER BY table_name
      `);

      // db.execute returns { rows: [...] } in drizzle-orm with postgres-js
      const rows = (result as any).rows || result;
      const existingTables = Array.isArray(rows) ? rows.map((r: any) => r.table_name) : [];

      const expectedTables = ['users', 'contacts', 'contact_agents', 'public_agents', 'tasks', 'forms', 'form_fields', 'form_submissions', 'form_responses', 'user_memory_facts'];
      const missingTables = expectedTables.filter(t => !existingTables.includes(t));

      // Get user count
      const userCountResult = await db.execute(sql`SELECT COUNT(*) as count FROM users`);
      const userCountRows = (userCountResult as any).rows || userCountResult;
      const userCount = Array.isArray(userCountRows) ? parseInt(userCountRows[0]?.count || '0') : 0;
    ...
    try {
      ...
      for (const key of agentKeys) {
        ...
        try {
          ...
          const existing = await db.execute(sql`SELECT agent_id FROM public_agents WHERE agent_id = ${agentId}`);
