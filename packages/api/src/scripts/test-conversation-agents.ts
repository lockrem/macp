...
interface TestScenario {
  name: string;
  description: string;
  userPrompt: string;  // What the "user simulator" should say
  expectedBehaviors: ExpectedBehavior[];
}

interface ExpectedBehavior {
  type: 'contact_extraction' | 'task_detection' | 'relationship_query' | 'normal_response';
  details?: string;
}
 ...
 */
async function analyzeForPerson(message: string, memories: string[]): Promise<{
  personMentioned: boolean;
  personName?: string;
  relationshipProvided?: string;  // Relationship provided in this message
  relationshipProvidedInMessage?: string;  // Alias
  relationshipFromMemory?: string;
  relationshipUnknown: boolean;
  couldInvolveTheirAgent: boolean;
}> {
 ...
 */
async function extractContacts(message: string, agentResponse: string): Promise<ExtractedPerson[]> {
  const response = await anthropic.messages.create({
    model: 'claude-sonnet-4-20250514',
    max_tokens: 800,
    temperature: 0.1,
 ...
 */
async function detectTask(message: string): Promise<{
  isTask: boolean;
  category?: string;
  description?: string;
} | null> {
