...
export interface IntroductionQuestion {
  ...
  question: string;
  followUp?: string;
  extractsMemory: string[];     // Memory categories this question extracts ["health", "medications"]
  extractsRules: boolean;       // Whether this question might extract preferences/rules
  priority: number;             // Order to ask questions (1 = first)
}
...
export interface MarketplaceMetadata {
  ...
  featured: boolean;
  shortDescription: string;      // One-line for marketplace cards
  longDescription: string;       // Full description for detail view
  capabilities: string[];        // Bullet points of what this agent can do
}
...
export interface SystemAgentTemplate {
  ...
  accentColor: string;          // UI color theme
  suggestedCategories: string[]; // Memory categories to suggest
  introductionGreeting: string; // Greeting for the introduction flow
  introductionQuestions: IntroductionQuestion[];  // Questions to ask during introduction
  marketplace: MarketplaceMetadata;  // Marketplace listing data
}
 ...
 */
export const SYSTEM_AGENT_TEMPLATES: SystemAgentTemplate[] = [
  {
 ...
 */
export function getSystemAgentTemplates(): SystemAgentTemplate[] {
 ...
 */
export function getTemplateById(templateId: string): SystemAgentTemplate | undefined {
 ...
 */
export function createAgentConfigsFromTemplates(
  ...
  return SYSTEM_AGENT_TEMPLATES.map(template => ({
 ...
 */
export function createSystemAgentsForUser(
  userId: string,
  provider: 'anthropic' | 'openai' | 'gemini' | 'groq' = 'anthropic'
): ReturnType<typeof templateToAgent>[] {
  return SYSTEM_AGENT_TEMPLATES.map(template =>
    templateToAgent(template, userId, provider)
  );
}
 ...
 * These are the flagship agents built by the MACP team
 */
export const MACP_CREATOR = {
  creatorId: 'macp',
  displayName: 'MACP',
  bio: 'The Multi-Agent Communication Platform team. Building the future of AI collaboration.',
  avatarUrl: undefined,
  verified: true,
  verificationBadges: ['official' as const],
  agentCount: SYSTEM_AGENT_TEMPLATES.length,
 ...
 */
export function templateToMarketplaceAgent(template: SystemAgentTemplate): {
  agentId: string;
  creatorId: string;
  creatorName: string;
  ...
  return {
    ...
    creatorId: MACP_CREATOR.creatorId,
    creatorName: MACP_CREATOR.displayName,
 ...
 */
export function getMACPOriginals(): ReturnType<typeof templateToMarketplaceAgent>[] {
  return SYSTEM_AGENT_TEMPLATES.map(templateToMarketplaceAgent);
}

/**
 * Gets MACP Originals by category
 */
export function getMACPOriginalsByCategory(category: string): ReturnType<typeof templateToMarketplaceAgent>[] {
  return SYSTEM_AGENT_TEMPLATES
 ...
 */
export function getFeaturedMACPOriginals(): ReturnType<typeof templateToMarketplaceAgent>[] {
  return SYSTEM_AGENT_TEMPLATES
