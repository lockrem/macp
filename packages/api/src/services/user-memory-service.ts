...
const FACT_CATEGORY_PATTERNS: Array<{
  category: string;
  patterns: RegExp[];
}> = [
  {
 ...
 */
function categorizeFact(factContent: string): string {
 ...
 */
function extractFactKey(factContent: string): string {
  const patterns = [
...
/**
 * Extract just the value from a fact sentence
 * e.g., "User's last name is Smith" -> "Smith"
 * e.g., "User prefers vegetarian food" -> "vegetarian food"
 */
function extractFactValue(factContent: string): string {
  const valuePatterns = [
    /(?:is|are|=|:)\s+(.+)$/i,  // "... is VALUE"
    /prefers?\s+(.+)$/i,        // "prefers VALUE"
    /(?:likes?|loves?|enjoys?)\s+(.+)$/i,  // "likes VALUE"
    /allergic\s+to\s+(.+)$/i,   // "allergic to VALUE"
    /has\s+(?:a|an)?\s*(.+)$/i, // "has VALUE"
  ];
 ...
 */
export async function getUserFacts(userId: string, category?: string): Promise<MemoryFact[]> {
  const db = getDatabase();

  const conditions = category
    ? and(eq(userMemoryFacts.userId, userId), eq(userMemoryFacts.category, category))
...
export async function upsertProfileFacts(
  userId: string,
  category: string,
  facts: Array<{ key: string; value: string | number | string[] | Record<string, unknown> }>
): Promise<number> {
  const db = getDatabase();
  const now = new Date();
  let saved = 0;
...
export async function deleteFact(userId: string, factId: string): Promise<boolean> {
  const db = getDatabase();
  const result = await db.delete(userMemoryFacts)
    .where(and(
 ...
 */
export async function saveFactsToUserMemory(
  userId: string,
  facts: string[],
  source: string
): Promise<number> {
  ...
  for (const factContent of facts) {
    ...
    const factValue = extractFactValue(factContent);  // Extract just the value, not the full sentence
    ...
    const existing = await db.select().from(userMemoryFacts)
      .where(and(
        eq(userMemoryFacts.userId, userId),
        eq(userMemoryFacts.category, category),
        eq(userMemoryFacts.key, factKey)
      ))
      .limit(1);
 ...
 */
export async function getFactsForPromptInjection(userId: string): Promise<string[]> {
  const facts = await getUserFacts(userId);
  return facts.map(f =>
