 ...
 */
export async function extractRulesFromConversation(
  conversationId: string,
  agentId: string,
  agentName: string,
...
Return a JSON array of extracted rules. If no rules found, return [].`;
  ...
  try {
    ...
    const extractedRules: ExtractedRule[] = JSON.parse(jsonMatch[0]);
 ...
 */
export async function extractRulesFromIntroduction(
  conversationId: string,
  agentId: string,
  agentName: string,
  messages: ConversationMessage[],
 ...
 */
export function extractedRulesToAgentRules(
