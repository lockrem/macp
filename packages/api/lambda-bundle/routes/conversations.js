 ...
 */
async function loadMemoryContext(userId, categories, _apiKey) {
    const db = (0, core_1.getDatabase)();
    const summaries = [];
    for (const category of categories) {
        try {
            const facts = await db.select().from(core_1.userMemoryFacts)
            ...
            if (facts.length > 0) {
                const displayName = category
                    ...
                    .join(' ');
                const factLines = facts.map(f => typeof f.value === 'string' ? f.value : `${f.key}: ${JSON.stringify(f.value)}`);
 ...
 */
async function storeExtractedFacts(userId, conversationId, extractionResult, _apiKey) {
    const db = (0, core_1.getDatabase)();
    const now = new Date();
    ...
    for (const [categoryName, facts] of Object.entries(extractionResult.factsByCategory)) {
        ...
        for (const fact of facts) {
            ...
            const existing = await db.select().from(core_1.userMemoryFacts)
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(core_1.userMemoryFacts.userId, userId), (0, drizzle_orm_1.eq)(core_1.userMemoryFacts.category, categoryName), (0, drizzle_orm_1.eq)(core_1.userMemoryFacts.key, fact.key)))
                .limit(1);
