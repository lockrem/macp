...
exports.listPublishedAgentsFromDB = listPublishedAgentsFromDB;
 ...
 */
async function getPublishedAgentFromDB(agentId) {
    try {
        ...
        return {
            ...
            introductionQuestions: row.introductionQuestions || [],
 ...
 */
async function savePublishedAgentToDB(agent) {
    try {
        const db = (0, core_1.getDatabase)();
        // Check if exists
        const existing = await db
            ...
            .from(core_1.publicAgents)
            .where((0, drizzle_orm_1.eq)(core_1.publicAgents.agentId, agent.agentId))
            .limit(1);
        const now = new Date();
        const values = {
 ...
 */
async function deletePublishedAgentFromDB(agentId) {
    try {
        const db = (0, core_1.getDatabase)();
        await db.delete(core_1.publicAgents).where((0, drizzle_orm_1.eq)(core_1.publicAgents.agentId, agentId));
 ...
 */
async function listPublishedAgentsFromDB(userId) {
    try {
        const db = (0, core_1.getDatabase)();
        const results = await db
            .select()
            .from(core_1.publicAgents)
            .where((0, drizzle_orm_1.eq)(core_1.publicAgents.ownerId, userId));
 ...
 */
async function getPublishedAgentFromS3(agentId) {
 ...
 */
async function savePublishedAgentToS3(agent) {
 ...
 */
async function deletePublishedAgentFromS3(agentId) {
    const key = `public-agents/${agentId}.json`;
