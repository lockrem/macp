...
// Helper Functions
// -----------------------------------------------------------------------------
function getAdapter(apiKey, provider = DEFAULT_PROVIDER) {
    switch (provider) {
        case 'anthropic':
            return (0, core_1.createClaudeAdapter)(apiKey, DEFAULT_MODEL);
        case 'openai':
            return (0, core_1.createOpenAIAdapter)(apiKey);
        case 'gemini':
            return (0, core_1.createGeminiAdapter)(apiKey);
        case 'groq':
            return (0, core_1.createGroqAdapter)(apiKey);
        default:
            return (0, core_1.createClaudeAdapter)(apiKey, DEFAULT_MODEL);
    }
}
 ...
 */
async function extractTask(input, apiKey, provider = DEFAULT_PROVIDER) {
 ...
 */
async function extractNames(input, apiKey, provider = DEFAULT_PROVIDER) {
    const adapter = getAdapter(apiKey, provider);
 ...
 */
async function matchContact(input, apiKey, provider = DEFAULT_PROVIDER) {
    const adapter = getAdapter(apiKey, provider);
 ...
 */
async function detectCompletion(input, apiKey, provider = DEFAULT_PROVIDER) {
    const adapter = getAdapter(apiKey, provider);
    const contextStr = input.conversationContext?.slice(-5).join('\n') || '(none)';
 ...
 */
async function extractFacts(input, apiKey, provider = DEFAULT_PROVIDER) {
    const adapter = getAdapter(apiKey, provider);
 ...
 */
async function runTestBatch(testCases, testFn, apiKey, provider = DEFAULT_PROVIDER) {
    const results = [];
