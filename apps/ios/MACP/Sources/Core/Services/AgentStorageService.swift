...
/// Users manually add agents from the Agent Store
@MainActor
class AgentStorageService: ObservableObject {
    ...
    func deleteAgent(_ agent: LocalAgent) {
        ...
        // Clean up server-side data and sync
        Task {
            // Delete all rules for this agent
            _ = await RulesService.shared.deleteAllRules(agentId: agentId)

            // Clear introduction status from local cache
            IntroductionService.shared.statusByAgent.removeValue(forKey: agentId)

            // Sync agent list to server
            await SettingsSyncService.shared.syncAgents()
        }
    }
