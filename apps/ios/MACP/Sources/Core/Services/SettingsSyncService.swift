...
class SettingsSyncService: ObservableObject {
    ...
    func syncFromServer() async {
        ...
        print("[Sync] Starting settings sync from server...")

        do {
            let response: SettingsResponse = try await APIClient.shared.get("/settings")

            print("[Sync] Got response, settings present: \(response.settings != nil)")

            if let settings = response.settings {
                print("[Sync] API keys present: \(settings.apiKeys != nil)")
                // Import API keys
                ...
                print("[Sync] No settings found on server (settings was nil)")
            }
        } catch APIError.unauthorized {
            // Token is invalid - user will need to sign in again
            print("[Sync] Authentication required - token may be expired")
        } catch let urlError as URLError {
            // Network error - don't show error, server might be temporarily unreachable
            print("[Sync] Network error syncing: \(urlError.localizedDescription)")
        } catch APIError.serverError {
            // Server error - don't bother user with this
            print("[Sync] Server error during sync (5xx)")
        } catch {
