...
class MemoryService: ObservableObject {
    ...
    func createSoloConversation(
        ...
        let payload: [String: Any] = [
            ...
            "agentConfig": [
                ...
                "systemPrompt": agent.systemPrompt  // Uses customSystemPrompt if set
            ] as [String: Any],
    ...
    // MARK: - Universal Chat (Orchestrated Routing)

    /// Creates a universal conversation with orchestrated agent routing
    func createUniversalConversation(
        apiKey: String,
        provider: String = "anthropic"
    ) async -> UniversalConversationResponse? {
        let payload: [String: Any] = [
            "apiKey": apiKey,
            "provider": provider,
            "extractFacts": true
        ...
        do {
            let url = URL(string: "\(APIClient.shared.baseURL)/conversations/universal")!
            var request = URLRequest(url: url)
            request.httpMethod = "POST"
            request.setValue("application/json", forHTTPHeaderField: "Content-Type")
            ...
            if let token = AuthService.shared.accessToken {
                ...
                print("[Memory] Warning: No access token available for universal conversation")
            }
            ...
            guard let httpResponse = response as? HTTPURLResponse else {
                print("[Memory] Universal conversation creation failed - invalid response")
                return nil
            }
            ...
            if !(200...299).contains(httpResponse.statusCode) {
                ...
                print("[Memory] Universal conversation creation failed - status: \(httpResponse.statusCode), body: \(errorBody)")
                return nil
            }

            return try decoder.decode(UniversalConversationResponse.self, from: data)
        } catch let urlError as URLError {
            print("[Memory] Network error creating universal conversation: \(urlError.localizedDescription)")
            return nil
        } catch {
            print("[Memory] Failed to create universal conversation: \(error)")
            return nil
        }
    }

    /// Sends a message in a universal conversation (orchestrated routing)
    func sendUniversalMessage(conversationId: String, content: String) async -> UniversalMessageResponse? {
        do {
            let response: UniversalMessageResponse = try await APIClient.shared.post(
                "/conversations/\(conversationId)/universal-message",
                body: ["content": content]
            )
            return response
        } catch {
            print("[Memory] Failed to send universal message: \(error)")
            return nil
        }
    }
}
